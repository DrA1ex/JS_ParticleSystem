...
The tree split strategy can be selected with `worker_mt_tree_strategy`. Supported values are `static`, `dynamic`, and `recursive`. `dynamic` is the default conservative queue-based scheduler. `static` is useful as a baseline. `recursive` is experimental: it starts from coarse regions and lets subworkers split heavy jobs further, returning spawned jobs back to the coordinator queue.
