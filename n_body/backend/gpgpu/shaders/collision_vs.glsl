...
void collide(vec3 p1, vec3 p2, vec3 p2Vel, float p2Mass) {
    vec3 deltaPos =  p1 - p2;
...
void main() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        vec4 atractor = texelFetch(particle_pos_mass_tex, index, 0).xyzw;
        vec3 atractor_velocity = texelFetch(particle_velocity_tex, index, 0).xyz;

        collide(position, atractor.xyz, atractor_velocity, atractor.w);
    }
