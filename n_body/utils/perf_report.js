...
import {BUILD_ID, WORKER_PROTOCOL_VERSION} from "./build.js";
...
function cloneWorkerMTState(state) {
    ...
    return {
        ...
        treeStrategy: state.treeStrategy || null,
        treeDynamicScheduling: !!state.treeDynamicScheduling,
        treeRecursiveScheduling: !!state.treeRecursiveScheduling,
        treeHybridScheduling: !!state.treeHybridScheduling,
        treeHybridProfile: state.treeHybridProfile || null,
        treeHybridSeedJobs: state.treeHybridSeedJobs ?? null,
        treeHybridSeedParallel: state.treeHybridSeedParallel ?? null,
        treeHybridSeedBoundsTime: finite(state.treeHybridSeedBoundsTime),
        treeHybridSeedCountTime: finite(state.treeHybridSeedCountTime),
        treeHybridSeedScatterTime: finite(state.treeHybridSeedScatterTime),
        treeSpawnedJobs: finite(state.treeSpawnedJobs),
        recursiveSplitBudget: finite(state.recursiveSplitBudget),
        recursiveMinJobParticles: finite(state.recursiveMinJobParticles),
        ...
        forceTimeTotal,
        integrateTimeTotal,
        forceKernel: state.forceKernel || null,
        forceKernelApplied: Array.isArray(state.forceKernelApplied) ? state.forceKernelApplied.slice() : [],
        forceKernelConsistent: state.forceKernelConsistent === true,
        forcePairChecks: finite(state.forcePairChecks),
        forceKernelTimeMax: finite(state.forceKernelTimeMax),
        forceKernelTimeTotal: finite(state.forceKernelTimeTotal),
        forceCollisionTimeMax: finite(state.forceCollisionTimeMax),
        forceCollisionTimeTotal: finite(state.forceCollisionTimeTotal),
        forceGatherTimeMax: finite(state.forceGatherTimeMax),
        forceGatherTimeTotal: finite(state.forceGatherTimeTotal),
        forcePairTimeMax: finite(state.forcePairTimeMax),
        forcePairTimeTotal: finite(state.forcePairTimeTotal),
        forceFlushTimeMax: finite(state.forceFlushTimeMax),
        forceFlushTimeTotal: finite(state.forceFlushTimeTotal),
        forceTimingSamples: finite(state.forceTimingSamples),
        treeFastBuild: state.treeFastBuild === true,
        treeFastBuildApplied: state.treeFastBuildApplied === true,
        treeFastBuildConsistent: state.treeFastBuildConsistent === true,
        treeFusedAggregate: state.treeFusedAggregate === true,
        treeFusedAggregateApplied: state.treeFusedAggregateApplied === true,
        treeFusedAggregateConsistent: state.treeFusedAggregateConsistent === true,
        treePartitionCountParticlesMax: finite(state.treePartitionCountParticlesMax),
        treePartitionCountParticlesTotal: finite(state.treePartitionCountParticlesTotal),
        treePartitionScatterParticlesMax: finite(state.treePartitionScatterParticlesMax),
        treePartitionScatterParticlesTotal: finite(state.treePartitionScatterParticlesTotal),
        treePartitionCountTimeMax: finite(state.treePartitionCountTimeMax),
        treePartitionCountTimeTotal: finite(state.treePartitionCountTimeTotal),
        treePartitionScatterTimeMax: finite(state.treePartitionScatterTimeMax),
        treePartitionScatterTimeTotal: finite(state.treePartitionScatterTimeTotal),
        treePartitionTimingSamples: finite(state.treePartitionTimingSamples),
        treeNodeInitCountMax: finite(state.treeNodeInitCountMax),
        treeNodeInitCountTotal: finite(state.treeNodeInitCountTotal),
        treeLeafCollectTimeMax: finite(state.treeLeafCollectTimeMax),
        treeLeafCollectTimeTotal: finite(state.treeLeafCollectTimeTotal),
        treeSingleBucketSplits: finite(state.treeSingleBucketSplits),
        treeSkippedScatterParticles: finite(state.treeSkippedScatterParticles),
        workerBuildId: state.workerBuildId || null,
        workerProtocolVersion: finite(state.workerProtocolVersion),
        taskWorkerBuildIds: Array.isArray(state.taskWorkerBuildIds) ? state.taskWorkerBuildIds.slice() : [],
        taskWorkerProtocolVersions: Array.isArray(state.taskWorkerProtocolVersions) ? state.taskWorkerProtocolVersions.slice() : [],
        workerCpuTime,
        workerMaxTime,
...
const BLOCK_METRICS = {
    ...
    "physics.mt.seedBounds": "physics.workerMT.treeHybridSeedBoundsTime",
    "physics.mt.seedCount": "physics.workerMT.treeHybridSeedCountTime",
    "physics.mt.seedScatter": "physics.workerMT.treeHybridSeedScatterTime",
    "physics.mt.treeSpawnedJobs": "physics.workerMT.treeSpawnedJobs",
    "physics.mt.treePartitionCountParticlesMax": "physics.workerMT.treePartitionCountParticlesMax",
    "physics.mt.treePartitionCountParticlesTotal": "physics.workerMT.treePartitionCountParticlesTotal",
    "physics.mt.treePartitionScatterParticlesMax": "physics.workerMT.treePartitionScatterParticlesMax",
    "physics.mt.treePartitionScatterParticlesTotal": "physics.workerMT.treePartitionScatterParticlesTotal",
    "physics.mt.treePartitionCountTimeMax": "physics.workerMT.treePartitionCountTimeMax",
    "physics.mt.treePartitionCountTimeTotal": "physics.workerMT.treePartitionCountTimeTotal",
    "physics.mt.treePartitionScatterTimeMax": "physics.workerMT.treePartitionScatterTimeMax",
    "physics.mt.treePartitionScatterTimeTotal": "physics.workerMT.treePartitionScatterTimeTotal",
    "physics.mt.treePartitionTimingSamples": "physics.workerMT.treePartitionTimingSamples",
    "physics.mt.treeNodeInitCountMax": "physics.workerMT.treeNodeInitCountMax",
    "physics.mt.treeNodeInitCountTotal": "physics.workerMT.treeNodeInitCountTotal",
    "physics.mt.treeLeafCollectMax": "physics.workerMT.treeLeafCollectTimeMax",
    "physics.mt.treeLeafCollectTotal": "physics.workerMT.treeLeafCollectTimeTotal",
    "physics.mt.treeSingleBucketSplits": "physics.workerMT.treeSingleBucketSplits",
    "physics.mt.treeSkippedScatterParticles": "physics.workerMT.treeSkippedScatterParticles",
    "physics.mt.forceMax": "physics.workerMT.forceTimeMax",
    "physics.mt.forcePairChecks": "physics.workerMT.forcePairChecks",
    "physics.mt.forceKernelMax": "physics.workerMT.forceKernelTimeMax",
    "physics.mt.forceKernelTotal": "physics.workerMT.forceKernelTimeTotal",
    "physics.mt.forceCollisionMax": "physics.workerMT.forceCollisionTimeMax",
    "physics.mt.forceCollisionTotal": "physics.workerMT.forceCollisionTimeTotal",
    "physics.mt.forceGatherMax": "physics.workerMT.forceGatherTimeMax",
    "physics.mt.forceGatherTotal": "physics.workerMT.forceGatherTimeTotal",
    "physics.mt.forcePairMax": "physics.workerMT.forcePairTimeMax",
    "physics.mt.forcePairTotal": "physics.workerMT.forcePairTimeTotal",
    "physics.mt.forceFlushMax": "physics.workerMT.forceFlushTimeMax",
    "physics.mt.forceFlushTotal": "physics.workerMT.forceFlushTimeTotal",
    "physics.mt.forceTimingSamples": "physics.workerMT.forceTimingSamples",
...
function snapshotSettings(app) {
    ...
    return {
        ...
        simulation: {
            ...
            workerMtTreeStrategy: settings.simulation.workerMtTreeStrategy,
            workerMtHybridProfile: settings.simulation.workerMtHybridProfile,
            workerMtHybridSeedJobs: settings.simulation.workerMtHybridSeedJobs,
            workerMtHybridSeedParallel: settings.simulation.workerMtHybridSeedParallel,
            workerMtHybridSplitFirst: settings.simulation.workerMtHybridSplitFirst,
            workerMtHybridJobSorting: settings.simulation.workerMtHybridJobSorting,
            workerMtHybridSplitGainFilter: settings.simulation.workerMtHybridSplitGainFilter,
            workerMtTreeFastBuild: settings.simulation.workerMtTreeFastBuild,
            workerMtTreeFusedAggregate: settings.simulation.workerMtTreeFusedAggregate,
            workerMtForceKernel: settings.simulation.workerMtForceKernel,
        },
        physics: {
...
function snapshotApp(app, collectorRafInterval, collectorTimestamp) {
    ...
    return {
        ...
        render: {
            ...
            effectiveColorMode: rendererStats.effectiveColorMode || null,
            colorFreezeStatus: rendererStats.colorFreezeStatus || null,
            colorFreezeFramesLeft: finite(rendererStats.colorFreezeFramesLeft),
...
function summarizeSlowFrames(samples, targetFrameTime) {
    ...
    return samples
        ...
        .map(sample => ({
            ...
            effectiveColorMode: sample.render?.effectiveColorMode || null,
            colorFreezeStatus: sample.render?.colorFreezeStatus || null,
...
function summarizeWorkerMTBlocks(blocks) {
    ...
    return {
        ...
        hybridSeedBoundsTime: summarizeAcrossBlocks(blocks, "physics.mt.seedBounds"),
        hybridSeedCountTime: summarizeAcrossBlocks(blocks, "physics.mt.seedCount"),
        hybridSeedScatterTime: summarizeAcrossBlocks(blocks, "physics.mt.seedScatter"),
        treeParallel: states.some(state => state.treeParallel),
        treeStrategies: uniqueValues(states.map(state => state.treeStrategy)),
        treeDynamicScheduling: states.some(state => state.treeDynamicScheduling),
        treeRecursiveScheduling: states.some(state => state.treeRecursiveScheduling),
        treeHybridScheduling: states.some(state => state.treeHybridScheduling),
        treeSpawnedJobs: summarizeAcrossBlocks(blocks, "physics.mt.treeSpawnedJobs"),
        treeFastBuildRequested: states.some(state => state.treeFastBuild),
        treeFastBuildApplied: states.some(state => state.treeFastBuildApplied),
        treeFastBuildConsistent: states.length > 0 && states.every(state => state.treeFastBuildConsistent),
        treeFusedAggregateRequested: states.some(state => state.treeFusedAggregate),
        treeFusedAggregateApplied: states.some(state => state.treeFusedAggregateApplied),
        treeFusedAggregateConsistent: states.length > 0 && states.every(state => state.treeFusedAggregateConsistent),
        treePartitionCountParticlesMax: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionCountParticlesMax"),
        treePartitionCountParticlesTotal: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionCountParticlesTotal"),
        treePartitionScatterParticlesMax: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionScatterParticlesMax"),
        treePartitionScatterParticlesTotal: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionScatterParticlesTotal"),
        treePartitionCountTimeMax: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionCountTimeMax"),
        treePartitionCountTimeTotal: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionCountTimeTotal"),
        treePartitionScatterTimeMax: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionScatterTimeMax"),
        treePartitionScatterTimeTotal: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionScatterTimeTotal"),
        treePartitionTimingSamples: summarizeAcrossBlocks(blocks, "physics.mt.treePartitionTimingSamples"),
        treeNodeInitCountMax: summarizeAcrossBlocks(blocks, "physics.mt.treeNodeInitCountMax"),
        treeNodeInitCountTotal: summarizeAcrossBlocks(blocks, "physics.mt.treeNodeInitCountTotal"),
        treeLeafCollectTimeMax: summarizeAcrossBlocks(blocks, "physics.mt.treeLeafCollectMax"),
        treeLeafCollectTimeTotal: summarizeAcrossBlocks(blocks, "physics.mt.treeLeafCollectTotal"),
        treeSingleBucketSplits: summarizeAcrossBlocks(blocks, "physics.mt.treeSingleBucketSplits"),
        treeSkippedScatterParticles: summarizeAcrossBlocks(blocks, "physics.mt.treeSkippedScatterParticles"),
        forceKernels: uniqueValues(states.map(state => state.forceKernel)),
        forceKernelsApplied: uniqueValues(states.flatMap(state => state.forceKernelApplied || [])),
        forceKernelConsistent: states.length > 0 && states.every(state => state.forceKernelConsistent),
        forcePairChecks: summarizeAcrossBlocks(blocks, "physics.mt.forcePairChecks"),
        forceKernelMax: summarizeAcrossBlocks(blocks, "physics.mt.forceKernelMax"),
        forceKernelTotal: summarizeAcrossBlocks(blocks, "physics.mt.forceKernelTotal"),
        forceCollisionMax: summarizeAcrossBlocks(blocks, "physics.mt.forceCollisionMax"),
        forceCollisionTotal: summarizeAcrossBlocks(blocks, "physics.mt.forceCollisionTotal"),
        forceGatherMax: summarizeAcrossBlocks(blocks, "physics.mt.forceGatherMax"),
        forceGatherTotal: summarizeAcrossBlocks(blocks, "physics.mt.forceGatherTotal"),
        forcePairMax: summarizeAcrossBlocks(blocks, "physics.mt.forcePairMax"),
        forcePairTotal: summarizeAcrossBlocks(blocks, "physics.mt.forcePairTotal"),
        forceFlushMax: summarizeAcrossBlocks(blocks, "physics.mt.forceFlushMax"),
        forceFlushTotal: summarizeAcrossBlocks(blocks, "physics.mt.forceFlushTotal"),
        forceTimingSamples: summarizeAcrossBlocks(blocks, "physics.mt.forceTimingSamples"),
        workerBuildIds: uniqueValues(states.map(state => state.workerBuildId)),
        workerProtocolVersions: uniqueValues(states.map(state => state.workerProtocolVersion)),
        taskWorkerBuildIds: uniqueValues(states.flatMap(state => state.taskWorkerBuildIds || [])),
        taskWorkerProtocolVersions: uniqueValues(states.flatMap(state => state.taskWorkerProtocolVersions || [])),
...
function buildCompactSummaryRows(reports) {
    return reports.map((report, index) => ({
        ...
        treeStrategy: report.comparisonKey?.workerMtTreeStrategy || report.settings?.simulation?.workerMtTreeStrategy || "n/a",
        hybridProfile: report.comparisonKey?.workerMtHybridProfile || report.settings?.simulation?.workerMtHybridProfile || "n/a",
        hybridSeedJobs: report.comparisonKey?.workerMtHybridSeedJobs || report.settings?.simulation?.workerMtHybridSeedJobs || "n/a",
        hybridSeedParallel: report.comparisonKey?.workerMtHybridSeedParallel ?? report.settings?.simulation?.workerMtHybridSeedParallel ?? "n/a",
        hybridSplitFirst: report.comparisonKey?.workerMtHybridSplitFirst ?? report.settings?.simulation?.workerMtHybridSplitFirst ?? "n/a",
        hybridJobSorting: report.comparisonKey?.workerMtHybridJobSorting ?? report.settings?.simulation?.workerMtHybridJobSorting ?? "n/a",
        hybridSplitGain: report.comparisonKey?.workerMtHybridSplitGainFilter ?? report.settings?.simulation?.workerMtHybridSplitGainFilter ?? "n/a",
        treeFastBuild: report.comparisonKey?.workerMtTreeFastBuild ?? report.settings?.simulation?.workerMtTreeFastBuild ?? "n/a",
        treeFusedAggregate: report.comparisonKey?.workerMtTreeFusedAggregate ?? report.settings?.simulation?.workerMtTreeFusedAggregate ?? "n/a",
        forceKernel: report.comparisonKey?.workerMtForceKernel ?? report.settings?.simulation?.workerMtForceKernel ?? "n/a",
        ...
        mtSpawned: fixed(metricAvg(report, "summary.workerMT.treeSpawnedJobs"), 0),
        ...
        mtCountParticles: fixed(metricAvg(report, "summary.workerMT.treePartitionCountParticlesMax"), 0),
        mtScatterParticles: fixed(metricAvg(report, "summary.workerMT.treePartitionScatterParticlesMax"), 0),
        mtCountTime: fixed(metricAvg(report, "summary.workerMT.treePartitionCountTimeMax"), 2),
        mtScatterTime: fixed(metricAvg(report, "summary.workerMT.treePartitionScatterTimeMax"), 2),
        mtFastApplied: report.summary?.workerMT?.treeFastBuildApplied ?? false,
        mtFastConsistent: report.summary?.workerMT?.treeFastBuildConsistent ?? false,
        mtFusedApplied: report.summary?.workerMT?.treeFusedAggregateApplied ?? false,
        mtFusedConsistent: report.summary?.workerMT?.treeFusedAggregateConsistent ?? false,
        mtForceKernelApplied: report.summary?.workerMT?.forceKernelsApplied?.join(",") || "",
        mtForceKernelConsistent: report.summary?.workerMT?.forceKernelConsistent ?? false,
        mtWorkerBuildIds: report.summary?.workerMT?.workerBuildIds?.join(",") || "",
        mtTaskBuildIds: report.summary?.workerMT?.taskWorkerBuildIds?.join(",") || "",
        mtWorkerProtocols: report.summary?.workerMT?.workerProtocolVersions?.join(",") || "",
        mtPairChecks: fixed(metricAvg(report, "summary.workerMT.forcePairChecks"), 0),
        mtForceKernel: fixed(metricAvg(report, "summary.workerMT.forceKernelMax"), 2),
        mtForceGather: fixed(metricAvg(report, "summary.workerMT.forceGatherMax"), 2),
        mtForcePair: fixed(metricAvg(report, "summary.workerMT.forcePairMax"), 2),
        mtForceFlush: fixed(metricAvg(report, "summary.workerMT.forceFlushMax"), 2),
        mtLeafCollect: fixed(metricAvg(report, "summary.workerMT.treeLeafCollectTimeMax"), 2),
        mtSingleBucket: fixed(metricAvg(report, "summary.workerMT.treeSingleBucketSplits"), 0),
        mtSkippedScatter: fixed(metricAvg(report, "summary.workerMT.treeSkippedScatterParticles"), 0),
...
export async function collectPerformanceReport(app, options = {}) {
    ...
    const report = {
        ...
        comparisonKey: {
            ...
            workerMtTreeStrategy: app.settings.simulation.workerMtTreeStrategy,
            workerMtHybridProfile: app.settings.simulation.workerMtHybridProfile,
            workerMtHybridSeedJobs: app.settings.simulation.workerMtHybridSeedJobs,
            workerMtHybridSeedParallel: app.settings.simulation.workerMtHybridSeedParallel,
            workerMtHybridSplitFirst: app.settings.simulation.workerMtHybridSplitFirst,
            workerMtHybridJobSorting: app.settings.simulation.workerMtHybridJobSorting,
            workerMtHybridSplitGainFilter: app.settings.simulation.workerMtHybridSplitGainFilter,
            workerMtTreeFastBuild: app.settings.simulation.workerMtTreeFastBuild,
            workerMtTreeFusedAggregate: app.settings.simulation.workerMtTreeFusedAggregate,
            workerMtForceKernel: app.settings.simulation.workerMtForceKernel,
            ...
            colorFreezeFrames: app.settings.render.colorFreezeFrames,
...
export function installPerformanceReportConsole(app) {
    ...
    window.nBodyReportHelp = () => {
        const message = [
            ...
            "  settings.simulation.workerMtHybridSeedJobs / workerMtHybridSeedParallel record the hybrid seed bootstrap configuration",
            "  settings.simulation.workerMtTreeFastBuild / workerMtTreeFusedAggregate record experimental worker tree options",
            "  settings.simulation.workerMtForceKernel records ordered / symmetric / symmetric-local leaf solving",
            "  summary.workerMT.treeFastBuildApplied / forceKernelsApplied show the code paths used by task workers",
            "  summary.workerMT.treeFastBuildConsistent / forceKernelConsistent confirm that requested and applied paths match",
            ...
            "  summary.workerMT.forcePairChecks / forceKernelMax / forceGatherMax / forcePairMax / forceFlushMax profile force kernels",
            "  summary.workerMT.treeSingleBucketSplits / treeSkippedScatterParticles show no-copy descent effectiveness",
            "  summary.workerMT.hybridSeedBoundsTime / hybridSeedCountTime / hybridSeedScatterTime show parallel bootstrap phases",
